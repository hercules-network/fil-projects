...
pub fn seal_extend(
    ...
    sector_id: SectorId,
    piece_lengths: &[UnpaddedBytesAmount],
) -> error::Result<SealOutputExtend> {
    println!("{:?} filecoin-proofs.api.seal_extend start", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let sector_bytes = usize::from(PaddedBytesAmount::from(porep_config));
    let in_data_tmp = in_data.clone();
    let in_tmp_dir = tempdir().unwrap();
    let mut in_save_path = in_tmp_dir.path().join(format!("{}.tmp", Uuid::new_v4().to_string())).display().to_string();
    let in_clean_path = in_save_path.clone();
    let in_open_path = in_save_path.clone();
    output(PathBuf::from(in_save_path), in_data_tmp.as_slice());
    let out_data_tmp = in_data.clone();
    let out_tmp_dir = tempdir().unwrap();
    let mut out_save_path = out_tmp_dir.path().join(format!("{}.tmp", Uuid::new_v4().to_string())).display().to_string();
    let out_clean_path = out_save_path.clone();
    let out_open_path = out_save_path.clone();
    output(PathBuf::from(out_save_path), out_data_tmp.as_slice());
    println!("{:?} filecoin-proofs.api.seal_extend section1", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    //let mut cur_data = in_data.clone();
    println!("{:?} filecoin-proofs.api.seal_extend sector_bytes: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string(), sector_bytes);
    println!("{:?} filecoin-proofs.api.seal_extend in_data: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string(), in_data.len());
    println!("{:?} filecoin-proofs.api.seal_extend piece_lengths: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string(), piece_lengths.len());
    let f_data = OpenOptions::new().read(true).write(true).open(&in_open_path)?;
    // Zero-pad the data to the requested size by extending the underlying file if needed.
    f_data.set_len(sector_bytes as u64)?;

    let mut data = unsafe { MmapOptions::new().map_mut(&f_data).unwrap() };
    println!("{:?} filecoin-proofs.api.seal_extend section2", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    println!("{:?} filecoin-proofs.api.seal_extend data: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string(), data.len());
    // Zero-pad the prover_id to 32 bytes (and therefore Fr32).
    let prover_id = pad_safe_fr(prover_id_in);
    // Zero-pad the sector_id to 32 bytes (and therefore Fr32).
    let sector_id_as_safe_fr = pad_safe_fr(&sector_id.as_fr_safe());
    let replica_id = replica_id::<DefaultTreeHasher>(prover_id, sector_id_as_safe_fr);
    println!("{:?} filecoin-proofs.api.seal_extend section 3", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let compound_setup_params = compound_proof::SetupParams {
        vanilla_params: &setup_params(
            PaddedBytesAmount::from(porep_config),
            usize::from(PoRepProofPartitions::from(porep_config)),
    ...
    let compound_public_params = ZigZagCompound::setup(&compound_setup_params)?;
    println!("{:?} filecoin-proofs.api.seal_extend section 4", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let (tau, aux) = ZigZagDrgPoRep::replicate(
        &compound_public_params.vanilla_params,
        &replica_id,
        &mut data,
    ...
    println!("{:?} filecoin-proofs.api.seal_extend section 5", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let mut source_src = OpenOptions::new().read(true).open(&out_open_path)?;
    let piece_specs = generate_piece_specs_from_source(&mut source_src, &piece_lengths)?;
    println!("{:?} filecoin-proofs.api.seal_extend section 7", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let piece_inclusion_proofs = piece_inclusion_proofs::<PedersenHasher>(&piece_specs, &aux[0])?;
    let comm_ps: Vec<Commitment> = piece_specs
        .iter()
        .map(|piece_spec| piece_spec.comm_p)
        .collect();
    println!("{:?} filecoin-proofs.api.seal_extend section 8", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    ...
    let groth_params = get_zigzag_params(porep_config)?;
    println!("{:?} filecoin-proofs.api.seal_extend section 9", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    info!(
        "got groth params ({}) while sealing",
        u64::from(PaddedBytesAmount::from(porep_config))
    );
    ...
    println!("{:?} filecoin-proofs.api.seal_extend section 10", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let mut buf = Vec::with_capacity(
        SINGLE_PARTITION_PROOF_LEN * usize::from(PoRepProofPartitions::from(porep_config)),
    );
    ...
    println!("{:?} filecoin-proofs.api.seal_extend section 12", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    if !valid_pieces {
        return Err(format_err!("pip verification sanity check failed"));
    }
    ...
    verify_seal(
        ...
        .expect("post-seal verification sanity check failed");
    println!("{:?} filecoin-proofs.api.seal_extend end", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    remove_file(in_clean_path);
    remove_file(out_clean_path);
    Ok(SealOutputExtend {
        comm_r,
        comm_r_star,
        comm_d,
...
pub fn generate_piece_commitment_extend(
    unpadded_piece_data: Vec<u8>,
    unpadded_piece_size: UnpaddedBytesAmount,
) -> error::Result<Commitment> {
    let mut source_cursor = Cursor::new(unpadded_piece_data);
    //let mut unpadded_piece_file = File::open(open_path)?;
    let mut padded_piece_file = tempfile()?;

    let (_, mut source) = get_aligned_source(&mut source_cursor, &[], unpadded_piece_size);
...
#[allow(dead_code)]
fn output(path: PathBuf, bytes: &[u8]) -> error::Result<()> {
    println!("{:?} filecoin-proofs.api.output start", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    let fp = match OpenOptions::new().truncate(true).create(true).write(true).open(Path::new(path.to_str().unwrap())) {
        Ok(f) => f,
        Err(e) => panic!("filecoin-proofs.api.output: {}", e)
    };
    let mut writer = BufWriter::with_capacity(1024 * 1024 * 4, fp);
    writer.write_all(bytes).unwrap();
    writer.write_all(&['\n' as u8]).unwrap();
    writer.flush().unwrap();
    println!("{:?} filecoin-proofs.api.output end", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    Ok(())
}
