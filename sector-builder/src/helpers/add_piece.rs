...
use crate::helpers::name_pipe::*;
use crate::helpers::file_cleanup::*;
use crate::nix;
...
#[allow(dead_code)]
fn output(path: PathBuf, data_in: Vec<u8>) {
    let pipe_write = NamedPipe::create_with_mode(path,Mode::S_IRWXU).unwrap();
    thread::spawn(move || block_on(async {
        let mut data = data_in.as_slice();
        pipe_write.write(data).await;
    }));
}
...
pub fn add_piece_extend(
    ...
    if let Some(s) = staged_state.sectors.get_mut(&dest_sector_id) {
        let piece_lengths: Vec<_> = s.pieces.iter().map(|p| p.num_bytes).collect();
        println!("{:?} sector-builder.add_piece_extend sector_access {}",chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string(), s.sector_access);
        let piece_lengths: Vec<_> = s.pieces.iter().map(|p| p.num_bytes).collect();
        let tmp_dir = tempdir().unwrap();
        let mut piece_path = tmp_dir.path().join(format!("{}.pipe",Uuid::new_v4().to_string()));
        //format!("/tmp/{}",&Uuid::new_v4().to_string());
        let pipe_write_path = piece_path.clone();
        let pipe_read_path = piece_path.clone();
        let clean_path = piece_path.clone();
        let mut cleanup = FileCleanup::new(&clean_path);
        output(pipe_write_path,piece_data);
        let (expected_num_bytes_written, mut chain) =
            get_aligned_source(File::open(pipe_read_path)?, &piece_lengths, piece_bytes_len);
        sector_store
            .manager()
            .write_and_preprocess(&s.sector_access, &mut chain)
            .map_err(Into::into)
